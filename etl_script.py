...
def fetch_strava():
    ...
    def store_segment_data(segment_data):
        response = supabase.table('athlete_segments').insert(segment_data).execute()
        return response

    def store_activity_data(activity_data):
        response = supabase.table('athlete_activities').insert(activity_data).execute()
        return response
    ...
    def clean_activities(all_activities):
        ...
        activities_df = pd.DataFrame(activity_data, columns=['athlete_id', 'activity_id', 'name', 'sport', 'type', 'datetime', 'distance', 'moving_time', 'elevation_gain', 'average_speed', 'average_heartrate', 'workout_type', 'map'])
    ...
    # Date range of activities to be pulled
    #start_date = '2023-01-01 '#'2024-06-03'
    #end_date = '2024-07-16' #start_timestamp=get_unix_timestamp(start_date)
...
def run_fact_activities(db_url, db_key, activities_df):
    ...
    def extract_summary_polyline(json_obj):
        return json_obj['summary_polyline']
    ...
    fact_columns = ['activity_id', 'athlete_id', 'name','datetime','distance', 'moving_time', 'elevation_gain', 'average_speed',
                    'average_heartrate','map','date_key','sport_type_key','race_key']
    ...
    fact_activity['map'] = fact_activity['map'].apply(extract_summary_polyline)
    fact_activity['map'] = fact_activity['map'].apply(polyline.decode)
...
def get_activity_laps(access_token, activity_id):
    ...
    if response.status_code == 200:
        ...
        # Prepare a list to store lap data
        lap_data = []
        
        if laps:
            for lap in laps:
                lap_number_match = re.search(r'(\d+)', lap['name'])
                lap_number = int(lap_number_match.group(1) if lap_number_match else 'Unknown')
                lap_type = 'Interval' if lap_number != 'Unknown' and lap_number % 2 != 0 else 'Rest'
                ...
                lap_info = {
                    ...
                    'max_speed': round(lap['max_speed'] * 3.6, 2),
                    'average_speed': round(lap['average_speed'] * 3.6, 2),
                }
            ...
            # Discard the first and last laps
            if len(df) > 2:  # Ensure there are at least 3 laps
                df_filtered = df.iloc[1:-1]
            else:
                df_filtered = pd.DataFrame()  # Return an empty DataFrame if not enough laps
            
            return df_filtered
        
        else:
            #print("No laps data found for this activity.")
            return pd.DataFrame()  # Return an empty DataFrame if no laps are found
...
@st.cache_data
def store_interval_data(intervals_df):
    if len(df_intervals) == 0:
        pass
    else:
        # Convert to dict and insert into fact table
        df_intervals_records = df_intervals.to_dict(orient='records')
        response = supabase.table('fact_intervals').insert(df_intervals_records).execute()
        print(f"Inserted{len(df_intervals_records)} new rows")